variable "project_id" {
  description = "Google Cloud project ID where the Cloud Run service is deployed."
  type        = string
}
...
variable "secret_names" {
  description = "Secret Manager secret names to expose as environment variables without storing secret values in Terraform state."
  type        = list(string)
  default     = []
}

variable "allow_public_access" {
  description = "Whether to grant allUsers permission to invoke the service."
  type        = bool
  default     = false
}
